...
uniform sampler2D uLogo1Texture;
uniform sampler2D uLogo2Texture;
...
void main() {
    ...
    vec4 logo1 = texture2D(uLogo1Texture, vUv);
    vec4 logo2 = texture2D(uLogo2Texture, vUv);
    vec4 disp = texture2D(uDisp, vUv);

     // go from purple logo to pink on hover
    float pct = clamp((disp.r - uHoverState) * 20. , 0., 1.);

    // go from pink logo to purple on hover
    // float pct = clamp((disp.r - uHoverState) * -20. , 0., 1.);

    gl_FragColor = mix(logo1, logo2, pct);
